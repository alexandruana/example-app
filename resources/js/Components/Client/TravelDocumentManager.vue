<template>
    <div>
        ...
        <!-- Add/Edit Form -->
        <div>
            <h2>{{ isEditMode ? 'Edit' : 'Add' }} Travel Document</h2>
            <form @submit.prevent="isEditMode ? updateDocument() : addDocument()">
            ...
            <table class="table-auto w-full">
                ...
                <tbody>
                    <tr v-for="document in travelDocuments" :key="document.id">
                        ...
                        <td class="border px-4 py-2">{{ document.country }}</td>
                        <td class="border px-4 py-2">{{ document.document_number }}</td>
                        <td class="border px-4 py-2">{{ document.expiry_date }}</td>
                        <td class="border px-4 py-2">{{ document.isDefault }}</td>
